...
$(document).ready(function() {
  ...
  window.addMarkerText = function addMarkerText(lat, long) {
    var message =
      ...
      '\');"></div><form id="addmark" method="post" action="https://api.apify.com/v2/acts/87ZoafhqSwzg8d7O5/runs?token=jfyMRD2LaBA4NTZmbcKHXxuW5"><select class="bg-white focus:outline-none focus:shadow-outline border border-gray-300 rounded-lg py-2 px-4 block w-full appearance-none leading-normal" name="icon" onchange="iconpref(this.value)">';
    for (var i in mapIcons) {
    ...
    $(".leaflet-popup-content")
      .append("<a href='#' class='leaflet-popup-close-button'>×</a>")
      .on("click", e => {
        map.closePopup();
      });
  ...
  const renderGeoSearch = (renderOptions, isFirstRendering) => {
    ...
    markers = items.map(
      ({ _geoloc, title, icon, description, category, location }) => {
        ...
        return L.marker(
          { lat: _geoloc.lat, lng: _geoloc.lng },
          { icon: iconUrl }
        )
          .bindPopup(
            "<b>" +
